...
def adj_voice(target):
    ...
    sf.write("out1.wav", pitch_corrected_y, sr)


    #Doubleing
    y, sr = librosa.load('out1.wav', sr=16000) # y is a numpy array of the wav file, sr = sample rate
    y_shifted = librosa.effects.pitch_shift(y, sr=sr, n_steps=STEPS) # shifted by 4 half steps

    sf.write("out2.wav", y_shifted, sr)

    #Combine
    sound1 = AudioSegment.from_file("out1.wav", format="wav")
    sound2 = AudioSegment.from_file("out2.wav", format="wav")
    ...
    # simple export
    overlay.export("out3.wav", format="wav")
    ...
    fx("./out3.wav", "voice_lines.wav")




    if os.path.exists("out1.wav"):
        os.remove("out1.wav")
    if os.path.exists("out2.wav"):
        os.remove("out2.wav")
    if os.path.exists("out3.wav"):
        os.remove("out3.wav")
